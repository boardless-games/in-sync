...
public class InputValidation {
  ...
  public static Optional<String> validateName(final String name) {
    if (name == null) {
      ...
      return Optional.of(String.format("Name must be at least %d characters long.", MIN_PLAYER_NAME_LENGTH));
    } else if (name.length() > MAX_PLAYER_NAME_LENGTH) {
      return Optional.of(String.format("Name cannot be longer than %d characters.", MAX_PLAYER_NAME_LENGTH));
  ...
  /**
   * Returns an Optional with a String explanation if the provided game code
   * is invalid. If valid, the Optional will be empty.
   */
